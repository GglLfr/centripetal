...
pub async fn apply_save(server: AssetServer, registry: AppTypeRegistry, ctx: AsyncContext, mut file: SaveData) -> Result {
    struct EntityMap<'a> {
    ...
    //let mut path_to_ids = HashMap::new();
                ...
                if let Some(map_asset_ids) = registry.get_type_data::<ReflectMapAssetIds>(component_type_id) {
                    map_asset_ids.map_asset_ids(&mut **component, &mut |id| match id {
                        UntypedAssetId::Index { type_id, index } => {
                            let Some(path) = file.asset_paths.get(&type_id).and_then(|type_paths| type_paths.get(&index)) else {
                                errors.push(format!("Couldn't map {id} into a known asset path, caused by malformed `SaveData`."));
                                return id
                            };

                            let Some(id) = server.get_path_ids(path).into_iter().find(|id| id.type_id() == type_id) else {
                                errors.push(format!(
                    ...
                    map_asset_ids.visit_asset_ids(&**component, &mut |id| {
                        if let UntypedAssetId::Index { type_id, index } = id {
                            let Some(path) = file.asset_paths.get(&type_id).and_then(|type_paths| type_paths.get(&index)) else {
                                errors.push(format!("Couldn't map {id} into a known asset path, caused by malformed `SaveData`."));
                                return
                            };
                            if !path_to_ids.contains(&path) {
...
pub fn apply_save_from_path(
    ...
    async move {
        let file = ctx
            ...
            .await?;
        apply_save(server, registry, ctx, file).await
    }
}
