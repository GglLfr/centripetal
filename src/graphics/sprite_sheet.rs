...
impl SpriteSection {
    ...
    pub fn sprite_with(&self, color: impl Into<Color>, size: impl Into<Option<Vec2>>, local_anchor: Anchor) -> Sprite {
        ...
        Sprite {
            ...
            rect: self.rect,
            anchor: Anchor::Custom(self.center_anchor.as_vec() + local_anchor.as_vec() * size / self.rect.map(|rect| rect.size()).unwrap_or(size)),
            image_mode: SpriteImageMode::Auto,
        }
    }
}
...
impl AssetLoader for SpriteSheetLoader {
    ...
    async fn load(&self, reader: &mut dyn Reader, _: &Self::Settings, load_context: &mut LoadContext<'_>) -> Result<Self::Asset, Self::Error> {
        ...
        let durations = frames.iter().map(|frame| Duration::from_millis(frame.duration as u64)).collect();
